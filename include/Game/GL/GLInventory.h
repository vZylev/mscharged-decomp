...
public:
    ...
    void Release()
    {
        freeing_GLInventory<ValueType> callback;
        m_pItems->Walk(
            &callback, &freeing_GLInventory<ValueType>::FreeItem);
        m_pItems->Clear();
    }
...
template <typename ValueType>
class deleting_GLInventoryContainer
{
public:
    typedef nlAVLTree<unsigned long, ValueType*,
        DefaultKeyCompare<unsigned long> > Tree;

    deleting_GLInventoryContainer()
    {
        m_pItems = new (8, false) Tree();
    }
    ...
    void Release()
    {
        deleting_GLInventory<ValueType> callback;
        m_pItems->Walk(
            &callback, &deleting_GLInventory<ValueType>::DeleteItem);
        m_pItems->Clear();
    }
...
public:
    ...
    void Release(void (*fn)(ValueType*))
    {
        if (fn != 0)
        {
            ...
            m_pItems->Walk(&callback, &Callback::Invoke);
        }
        m_pItems->Clear();
    }
...
public:
    ...
    /* 0x044 */ freeing_GLInventoryContainer<nlChunk>* m_pSkinData[16];
    /* 0x084 */ clearing_GLInventory<glModel>* m_pModels[16];
    /* 0x0C4 */ clearing_GLInventory<GLTextureAnim>* m_pTextureAnims[16];
    /* 0x104 */ deleting_GLInventoryContainer<GLVertexAnim>* m_pVertexAnims[16];
