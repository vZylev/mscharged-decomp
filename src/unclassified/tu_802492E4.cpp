...
struct TU8024A368HistoryRecord
{
    ...
    unsigned int mUnidentified0A : 3;
...
extern "C" const char* fn_801CAA20(const CharacterInfo* character);
extern "C" void fn_801CA610(TLInstance* instance, bool visible);
extern "C" TLTextInstance* fn_801CF97C(TLSlide* slide,
...
extern "C" TLImageInstance* fn_801E129C(TLSlide* slide,
    InlineHasher Level1, InlineHasher Level2, InlineHasher Level3,
    InlineHasher Level4 = InlineHasher(0UL), InlineHasher Level5 = InlineHasher(0UL),
    InlineHasher Level6 = InlineHasher(0UL));
...
extern "C" void fn_8024D074(TU8024A368Scene* scene)
{
    int value0 = scene->mHistory[scene->mUnidentified30C].mUnidentified00;
    int value1 = scene->mHistory[scene->mUnidentified30C].mUnidentified04;
    int value2 = scene->mHistory[scene->mUnidentified30C].mUnidentified07;
    int value3 = scene->mHistory[scene->mUnidentified30C].mUnidentified0A;
    WideBasicString string;
    switch (value0)
    {
    case 0:
        string = fn_801CA950(g_pLocalization, "NAME_MARIO");
        break;
    case 1:
        string = fn_801CA950(g_pLocalization, "NAME_BOWSER");
        break;
    case 2:
        string = fn_801CA950(g_pLocalization, "NAME_DAISY");
        break;
    case 3:
        string = fn_801CA950(g_pLocalization, "NAME_DK");
        break;
    case 4:
        string = fn_801CA950(g_pLocalization, "NAME_LUIGI");
        break;
    case 5:
        string = fn_801CA950(g_pLocalization, "NAME_PEACH");
        break;
    case 6:
        string = fn_801CA950(g_pLocalization, "NAME_WALUIGI");
        break;
    case 7:
        string = fn_801CA950(g_pLocalization, "NAME_WARIO");
        break;
    case 8:
        string = fn_801CA950(g_pLocalization, "NAME_YOSHI");
        break;
    case 9:
        string = fn_801CA950(g_pLocalization, "NAME_BOWSERJR");
        break;
    case 10:
        string = fn_801CA950(g_pLocalization, "NAME_DIDDYKONG");
        break;
    case 11:
        string = fn_801CA950(g_pLocalization, "NAME_PETEY");
        break;
    }

    FEPresentation* presentation = fn_801CF848(scene->mFEScene)->GetPresentation();
    TLTextInstance* text = fn_801CF97C(fn_801CA63C(presentation),
        "Layer", "HISTORY", "NAMES", "NAMES");
    memcpy(scene->mUnidentified4EC, fn_801CF604(&string), sizeof(scene->mUnidentified4EC));
    text->SetString(scene->mUnidentified4EC);

    const CharacterInfo& captain = GetCharacterInfo(GetCharacterIndexFromCaptain(value0));
    TLImageInstance* image;
    char path[0x80];

    image = fn_801E129C(fn_801CA63C(presentation), "Layer", "HISTORY", "CAPTAIN");
    scene->mImages[0]->fn_8024F4D8(image);
    fn_801CA610(image, false);
    nlSNPrintf(path, sizeof(path), "fe/hof_history_images/hof_history_%s", fn_801CAA20(&captain));
    scene->mImages[0]->QueueLoad(path, false);

    image = fn_801E129C(fn_801CA63C(presentation), "Layer", "HISTORY", "KRITTER");
    scene->mImages[4]->fn_8024F4D8(image);
    fn_801CA610(image, false);
    nlSNPrintf(path, sizeof(path), "fe/hof_history_images/hof_history_kritter_%s", fn_801CAA20(&captain));
    scene->mImages[4]->QueueLoad(path, false);

    const CharacterInfo& sidekick1 = GetCharacterInfo(GetCharacterIndexFromSidekick(value1));
    image = fn_801E129C(fn_801CA63C(presentation), "Layer", "HISTORY", "SK1");
    scene->mImages[1]->fn_8024F4D8(image);
    fn_801CA610(image, false);
    nlSNPrintf(path, sizeof(path), "fe/hof_history_images/hof_history_sidekicks_%s_%s",
        fn_801CAA20(&sidekick1), fn_801CAA20(&captain));
    scene->mImages[1]->QueueLoad(path, false);

    const CharacterInfo& sidekick2 = GetCharacterInfo(GetCharacterIndexFromSidekick(value2));
    image = fn_801E129C(fn_801CA63C(presentation), "Layer", "HISTORY", "SK2");
    scene->mImages[2]->fn_8024F4D8(image);
    fn_801CA610(image, false);
    nlSNPrintf(path, sizeof(path), "fe/hof_history_images/hof_history_sidekicks_%s_%s",
        fn_801CAA20(&sidekick2), fn_801CAA20(&captain));
    scene->mImages[2]->QueueLoad(path, false);

    const CharacterInfo& sidekick3 = GetCharacterInfo(GetCharacterIndexFromSidekick(value3));
    image = fn_801E129C(fn_801CA63C(presentation), "Layer", "HISTORY", "SK3");
    scene->mImages[3]->fn_8024F4D8(image);
    fn_801CA610(image, false);
    nlSNPrintf(path, sizeof(path), "fe/hof_history_images/hof_history_sidekicks_%s_%s",
        fn_801CAA20(&sidekick3), fn_801CAA20(&captain));
    scene->mImages[3]->QueueLoad(path, false);
