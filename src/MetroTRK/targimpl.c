...
DSError TRKTargetAddStopInfo(MessageBuffer* b)
{
    ...
    TRKTargetReadInstruction((void*)&instruction, gTRKCPUState.Default.PC);
    buf[3] = instruction;
...
void TRKTargetAddExceptionInfo(MessageBuffer* b)
{
    ...
    TRKTargetReadInstruction((void*)&local_54, gTRKExceptionStatus.exceptionInfo.PC);
...
DSError TRKTargetSupportRequest(void)
{
    ...
    {
        err = HandleOpenFileSupportRequest((char*)gTRKCPUState.Default.GPR[4], (u8)gTRKCPUState.Default.GPR[5], (u32*)gTRKCPUState.Default.GPR[6], &io_result);
    ...
    else if (command == kDSPositionFile)
    {
        ...
        err = HandlePositionFileSupportRequest((DSFileHandle)gTRKCPUState.Default.GPR[4], (u32*)&msg_length, (u8)gTRKCPUState.Default.GPR[6], &io_result);
    ...
    else
    {
        ...
        err = TRKSuppAccessFile((DSFileHandle)gTRKCPUState.Default.GPR[4], (u8*)gTRKCPUState.Default.GPR[6], length, &io_result, 1, (command == kDSReadFile));
...
{
    // all nop by default
    InstructionType instructionData[] = { INSTR_NOP, INSTR_NOP, INSTR_NOP, INSTR_NOP, INSTR_NOP, INSTR_NOP, INSTR_NOP, INSTR_NOP, INSTR_NOP, INSTR_NOP };
...
DSError TRKPPCAccessFPRegister(void* srcDestPtr, u32 fpr, bool read)
{
    DSError error = kNoError;
    // all nop by default
    InstructionType instructionData1[] = { INSTR_NOP, INSTR_NOP, INSTR_NOP, INSTR_NOP, INSTR_NOP, INSTR_NOP, INSTR_NOP, INSTR_NOP, INSTR_NOP, INSTR_NOP };
