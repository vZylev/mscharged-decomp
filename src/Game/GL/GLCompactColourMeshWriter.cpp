...
GLCompactColourMeshWriter::GLCompactColourMeshWriter()
    : count(0)
    , model(0)
    , resource(0)
    , position(0)
...
bool GLCompactColourMeshWriter::Begin(
    int vertexCount, int primitive, void* allocator)
{
    ...
    glCreateModel(
        model, vertexCount, primitive, allocator, 3, 0x4ED6C66F);
    ...
    else
    {
        if (allocator != 0)
        {
            positionData = (short*)glResourceAlloc(
                positionCount * sizeof(short), GLM_VertexData, allocator);
        }
        else
        {
            positionData = (short*)glFrameAlloc(
                positionCount * sizeof(short), GLM_VertexData);
        }
    }
    position = positionData;
    glSetModelStream(streams, 0, position, sizeof(short) * 3, 1);
...
bool GLCompactColourMeshWriter::End()
{
