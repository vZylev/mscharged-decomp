...
RVL_LIB_VERSION(HBM, "Dec  7 2006", "20:57:28", "0x4199_60831");
...
OSMutex homebutton::HomeButton::sMutex;
...
void HomeButton::initSound(const char* path)
{
    ...
    void* pvVar4 = MEMAllocFromAllocator(&sSoundAllocator, sizeof(nw4hbm::snd::MemorySoundArchive));
    if (pvVar4 != NULL)
    {
        mpMemorySoundArchive = new (pvVar4) nw4hbm::snd::MemorySoundArchive();
    }

    NW4HBMAssert_Line(mpMemorySoundArchive, LN(3941, 3872));
    NW4HBMAssertMessage_Line(mpMemorySoundArchive->Setup(path), LN(3946, 3877), "Cannot setup MemorySoundArchive");
...
const char* HomeButton::scFuncPaneName[5] = { "let_icn_00", "N_plyr_00", "N_plyr_01", "N_plyr_02", "N_plyr_03" };
...
void HomeButton::create()
{
    ...
    NW4HBMAssert_Line(mpResAccessor, 629);
    ...
    if (!mpHBInfo->cursor)
    {
        for (i = 0; i < (int)ARRAY_COUNT(mpCursorLayout); i++)
        {
            ...
            NW4HBMAssert_Line(mpCursorLayout[i], 641);
    ...
    for (nw4hbm::lyt::PaneList::Iterator it = pRootPane->GetChildList().GetBeginIter();
         it != pRootPane->GetChildList().GetEndIter(); ++it)
    {
        if (strcmp(it->GetName(), "back_01") == 0)
        {
    ...
    for (i = 0; i < mAnmNum; i++)
    {
        ...
        NW4HBMAssertPointerNonnull_Line(lpaRes, 683);
        ...
        NW4HBMAssert_Line(mpAnmController[i], 689);
        ...
        mpAnmController[i]->init(ANIM_TYPE_FORWARD, mpAnmController[i]->mpAnimGroup->GetFrameMax(), 0, mpHBInfo->frameDelta);
    }
    ...
    for (i = 0; i < (int)ARRAY_COUNT(mpGroupAnmController); i++)
    {
        ...
        NW4HBMAssertPointerNonnull_Line(lpaRes, 715);
        ...
        NW4HBMAssert_Line(mpGroupAnmController[i], 721);
        ...
        mpGroupAnmController[i]->init(ANIM_TYPE_FORWARD, mpGroupAnmController[i]->mpAnimGroup->GetFrameMax(), 0, mpHBInfo->frameDelta);
    }
    ...
    for (i = 0; i < (int)ARRAY_COUNT(mpPairGroupAnmController); i++)
    {
        ...
        NW4HBMAssertPointerNonnull_Line(lpaRes, 747);
        ...
        NW4HBMAssert_Line(mpPairGroupAnmController[i], 753);
        ...
        mpPairGroupAnmController[i]->init(ANIM_TYPE_FORWARD, mpPairGroupAnmController[i]->mpAnimGroup->GetFrameMax(), 0, mpHBInfo->frameDelta);
    }
    ...
    NW4HBMAssert_Line(mpHomeButtonEventHandler, 776);

    if (void* pMem = HBMAllocMem(sizeof(*mpPaneManager)))
    {
        mpPaneManager = new (pMem) gui::PaneManager(mpHomeButtonEventHandler, NULL, spAllocator);
    }

    NW4HBMAssert_Line(mpPaneManager, 783);
...
void HomeButton::set_config()
{
    ...
    NW4HBMAssert_Line(mpLayoutName, 847);
    ...
    NW4HBMAssert_Line(mpAnmName, 857);
...
void HomeButton::updateTrigPane()
{
    ...
    case eSeq_Normal:
        for (i = 0; i < (int)ARRAY_COUNT(scFuncTouchPaneName); i++)
        {
            if (i < 2 || i == 9)
            {
                nw4hbm::lyt::Pane* pTouchPane = mpLayout->GetRootPane()->FindPaneByName(scFuncTouchPaneName[i], true);
...
bool HomeButton::isDownBarActive()
{
    ...
    if (!isActive() || mpGroupAnmController[anm_no[0]]->isPlaying() || mpGroupAnmController[anm_no[1]]->isPlaying() || mpGroupAnmController[anm_no[2]]->isPlaying() || mpGroupAnmController[anm_no[3]]->isPlaying())
    {
        flag = false;
    }
...
int HomeButton::getPaneNo(const char* panename)
{
    ...
    {
        if (!strcmp(panename, getPaneName(i)))
        {
            ret = i;
            break;
        }
    ...
    for (int i = 0; i < 10; i++)
    {
        if (!strcmp(panename, getFuncPaneName(i)))
        {
            ret = i + buttonNum;
            break;
        }
...
void HomeButton::createSound(nw4hbm::snd::SoundArchive* pSoundArchive, bool bCreateSoundHeap)
{
    ...
    NW4HBMAssert_Line(mpSoundArchivePlayer, LN(3752, 3714));
    ...
    NW4HBMAssert_Line(mpSoundHandle, LN(3777, 3739));
