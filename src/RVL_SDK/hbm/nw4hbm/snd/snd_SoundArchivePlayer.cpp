...
const SoundArchive& SoundArchivePlayer::GetSoundArchive() const {
    NW4HBMAssertMessage_Line(mSoundArchive != NULL, 684, "Setup is not completed.");
    return *mSoundArchive;
}

SoundPlayer& SoundArchivePlayer::GetSoundPlayer(const char* label) {
    NW4HBMAssertPointerNonnull(mSoundArchive);
    return GetSoundPlayer(mSoundArchive->ConvertLabelStringToPlayerId(label));
}
...
void SoundArchivePlayer::StrmHeaderLoadTask::Execute() {
    static u8 buffer[STREAM_BUFFER_SIZE] ALIGN(32);
