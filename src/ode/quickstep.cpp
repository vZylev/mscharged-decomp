    ...
    dxJoint* const* _joint, int nj, dReal stepsize)
{
    ...
    if (m > 0)
    {
        ...
        for (i = 0; i < nb; i++)
        {
            ...
            for (j = 0; j < 3; j++)
                tmp1[i * 6 + j] = body_invMass * body[i]->facc[j]
                    + stepsize1 * body[i]->lvel[j];
            dMULTIPLY0_331(tmp1 + i * 6 + 3, invI + i * 12, body[i]->tacc);
            for (j = 0; j < 3; j++)
                tmp1[i * 6 + 3 + j] += stepsize1 * body[i]->avel[j];
        }
        ...
        SOR_LCP(m, nb, J, jb, body, invI, lambda, cforce, rhs, lo, hi, cfm, findex,
            &world->qs);
