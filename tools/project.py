...
class ProjectConfig:
    def __init__(self) -> None:
        ...
        self.linker_alignment_splits: Optional[Path] = None  # DOL input alignments
...
def generate_build_ninja(
    ...
    n.rule(
        name="link_order",
        command=(
            '$python tools/link_order.py --manifest "$manifest" '
            '--splits "$splits" --script "$script" --output "$ldscript"'
        ),
        description="ALIGN $ldscript",
    )
    n.newline()
    ...
    class LinkStep:
        ...
        def add(self, obj: Path, name: str) -> None:
        ...
        def write(self, n: ninja_syntax.Writer) -> None:
            ...
            if self.module_id == 0:
                ...
                if config.linker_alignment_splits is not None:
                    link_dir = build_path / "link" / self.name
                    link_dir.mkdir(parents=True, exist_ok=True)
                    manifest_path = link_dir / "inputs.json"
                    entries = [
                        {
                            "unit": name,
                            "source": src,
                            "output": serialize_path(
                                link_dir / f"{i:04d}_{Path(src).name}"
                            ),
                        }
                        for i, (name, src) in enumerate(zip(self.unit_names, inputs))
                    ]
                    ...
                    ldscript = link_dir / "aligned.lcf"
                    aligned_inputs = [entry["output"] for entry in entries]
                    n.build(
                        outputs=ldscript,
                        implicit_outputs=aligned_inputs,
                        rule="link_order",
                        inputs=inputs,
                        implicit=[
                            self.ldscript,
                            manifest_path,
                            config.linker_alignment_splits,
                            "tools/link_order.py",
                        ],
                        variables={
                            "manifest": manifest_path,
                            "splits": config.linker_alignment_splits,
                            "script": self.ldscript,
                            "ldscript": ldscript,
                        },
                    )
        ...
        def add_unit(build_obj: BuildConfigUnit, link_step: LinkStep):
            ...
            if obj is None:
                ...
                if obj_path is not None:
                    link_step.add(Path(obj_path), obj_name)
                return
            ...
            if link_built_obj and built_obj_path is not None:
                ...
                link_step.add(built_obj_path, obj_name)
